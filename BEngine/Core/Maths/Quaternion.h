...
public:
    ...
    static inline Quaternion AxisRotation ( const float& angleInDegrees, const Vector3& axis )
    {
        ...
        float cosHalfAngle = cos ( degToRad );
        float sinHalfAngle = sin ( degToRad );
    ...
    {
        Quaternion z = lhs;
        Quaternion zConjugate = Quaternion::Conjugate ( lhs );
        Quaternion vecAsQuaternion = Quaternion ( 0, vec.x, vec.y, vec.z );

        float quadrant = Quaternion::Quadrant ( z );

        Quaternion step1 = (z * vecAsQuaternion);
        Quaternion step2 = ( step1 * zConjugate);
        Quaternion result = step2 / quadrant;
